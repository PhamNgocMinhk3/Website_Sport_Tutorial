...
function Arena() {
    ...
    useEffect(() => {
        axios.get("http://localhost:3000/sanA").then(response => {
    ...
    <div className={cx('Arena-container')}>
        ...
        <div className={cx('Arena-Content-menu')}>
            ...
            {san.map((san, index)  => (
                    <div className={cx('Arena-stadium-content')} key={index}>
                        <img className={cx('Arena-avata-img')} src={`http://localhost:3000/${san.Logo_1}`}alt=''/>
                        <ul  className={cx('Arena-stadium-list')}>{san.TenSan}
                            <li className={cx('Arena-stadium-item')}>Cụm Sân: {san.TenCumSan}</li>
                            <li className={cx('Arena-stadium-item')}>Loại: {san.TenLoaiSan}</li>
                            <div className={cx('Arena-stadium-link')}>
                                <button  className={cx('Stadium-link-button')}>
                                    <Link to={`/yarddetails/${san.ID}`}>Xem Chi tiết</Link>
                                    </button>
                            </div>
                        </ul>
                    </div>
